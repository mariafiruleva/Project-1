...
workDir <- "~/Bioinformatics/Practice/rna-seq/sart"      # working directory for the R session

projectName <- "RNA_expression_in_bread"                         # name of the project
author <- "Maria Firuleva"                                # author of the statistical analysis/report

targetFile <- "target_file"                           # path to the design/target file
rawDir <- "~/Bioinformatics/Practice/rna-seq/sart"                                      # path to the directory containing raw counts files
featuresToRemove <- c("alignment_not_unique",        # names of the features to be removed
                      "ambiguous", "no_feature",     # (specific HTSeq-count information and rRNA for example)
                      "not_aligned", "too_low_aQual")# NULL if no feature to remove

varInt <- "group"                                    # factor of interest
condRef <- "0minutes"                                      # reference biological condition
